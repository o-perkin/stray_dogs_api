  ...
  context "Post/PATCH requests" do
    ...
    describe "Post /api/v1/subscribes" do
      it "sends status ok and new subscribe info" do
        ...
        city = 1
        sign_in user
        post '/api/v1/subscribes', params: { subscribe: { subscriptions_attributes: {"0" => {breed: breed, city: city, age_from: "1", age_to: "2", _destroy: "false"}} } }
      ...
      it "sends status 422 and error that you've already subscribed" do
        ...
        expect(response).to have_http_status(200)
        sign_in user
        post '/api/v1/subscribes', params: { subscribe: { subscriptions_attributes: {"0" => {breed: breed, city: city, age_from: "2", age_to: "4", _destroy: "false"}} } }
      ...
      it "ends error when age_to less than age_from" do
        ...
        city = 1
        sign_in user
        post '/api/v1/subscribes', params: { subscribe: { subscriptions_attributes: {"0" => {breed: breed, city: city, age_from: "2", age_to: "1", _destroy: "false"}} } }
    ...
    describe "PATCH /api/v1/subscribes" do
      it "sends status ok and updated subscribe's info" do
        ...
        patch "/api/v1/subscribes/#{subscribe.id}", params: { subscribe: { subscriptions_attributes: {"0" => {breed: breed, city: city, age_from: "1", age_to: "3", _destroy: "false"}} } }
        
        expect(response).to have_http_status(200)
        expect(json['message']).to eq("Subscribe updated")
        expect(json['data'].last["breed"]).to eq(breed)        
      end
      ...
      it "sends error when one of params are empty" do
        ...
        sign_in user
        subscribe = create(:subscribe, user_id: user.id)
        patch "/api/v1/subscribes/#{subscribe.id}", params: { subscribe: { subscriptions_attributes: {"0" => {breed: "", city: city, age_from: "1", age_to: "3", _destroy: "false"}} } }
        
        expect(response).to have_http_status(422)
        expect(json['message']).to eq("Subscribe not updated")
        expect(json['data']["subscriptions.breed"]).to eq(["can't be blank"])        
      end
      ...
      it "sends error when age_to less than age_from" do
        ...
        sign_in user
        subscribe = create(:subscribe, user_id: user.id)
        patch "/api/v1/subscribes/#{subscribe.id}", params: { subscribe: { subscriptions_attributes: {"0" => {breed: 1, city: city, age_from: "2", age_to: "1", _destroy: "false"}} } }
