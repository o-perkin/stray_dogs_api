...
export default function MyDogsPage(props) {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:3001/api/v1/my_dogs`, {
        headers: {
          Accept: '*/*',
  ...
  function loadDogs() {
    axios
      .get(
        `http://localhost:3001/api/v1/my_dogs?page=${latestPage.current}&breed=${latestFilters.current.breed}&city=${latestFilters.current.city}&age_from=${latestFilters.current.age_from}&age_to=${latestFilters.current.age_to}&sort=${latestSorting.current.sort}&direction=${latestSorting.current.direction}`,
        {
          headers: {
            Accept: '*/*',
            Authorization: localStorage.getItem('token')
